            ...
            while SkinOpen do
              Citizen.Wait(1)
                ...
                RageUI.IsVisible(SkinOpenMenu, function()
                        RageUI.List("Type de caméra", {"Proche", "Normal", "Éloignez", "Visages", "Haut", "Bas", "Chaussure"}, ClothesIndexCam, "Vous pouvez appuyer sur [~b~ESPACE~s~] pour tourner votre personnage.", {}, true, {
                        ...
                        for k,v in pairs(comp) do
                            if v.table[1] ~= nil then
                                RageUI.List(v.label, v.table, i[v.name], "Vous pouvez appuyer sur [~b~ESPACE~s~] pour tourner votre personnage.", {}, true, {
                                    onSelected = function()
                                        RageUI.CloseAll()
                                        SkinOpen = false
                                        RenderScriptCams(0, 1, 1200, 0, 0)
                                        ...
                                        FreezeEntityPosition(PlayerPedId(), false)
                                        ESX.ShowAdvancedNotification("Notification", "OneLife", "Votre personnage a été sauvegarder avec succès !", "CHAR_ST", 1)
                                    end,
                                    onListChange = function(index, item)
                                        if index ~= nil then
                                            refreshDataSkin()
