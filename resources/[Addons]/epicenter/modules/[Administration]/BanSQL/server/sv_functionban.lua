...
function banplayer(source, args)
								...
								local content = {
									{
										["title"] = "**Logs Ban :**",
										["fields"] = {
											{ name = "- Joueurs ban :", value = pseudo.." ["..target.."] ["..license.."]" },
...
function banplayeroffline(source, args)
    if args ~= "" then
        local ReasonForBan     = table.concat(args, " ", 3)
        local target           = args[1] -- Identifiant du joueur (par ex. licence)
        local duree            = tonumber(args[2]) -- Durée du ban
        local sourceplayername = source ~= 0 and GetPlayerName(source) or "Console"

        -- Vérifications simples des arguments
        if not target or target == "" then
            TriggerClientEvent("esx:showNotification", source, "~r~Administration\n~s~ID invalide (aucune licence fournie).")
            return
        end

        if not duree or duree == "" or duree < 0 then
            TriggerClientEvent("esx:showNotification", source, "~r~Administration\n~s~Durée invalide.")
            return
        end

        if ReasonForBan == "" then
            ReasonForBan = Text.noreason -- Utilisez une raison par défaut si aucune n'est donnée
        end
        ...
        MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier = @identifier', 
        {
            ...
            if data and data[1] then
                -- L'utilisateur existe, effectuer le ban
                if duree > 0 then
                    -- Bannissement temporaire
                    banOff(source, target, nil, nil, nil, nil, nil, data[1].name or "Nom inconnu", sourceplayername, duree, ReasonForBan, 0)
                else
                    -- Bannissement permanent
                    banOff(source, target, nil, nil, nil, nil, nil, data[1].name or "Nom inconnu", sourceplayername, duree, ReasonForBan, 1)
                end

                TriggerClientEvent("esx:showNotification", source, "~g~Le joueur a été banni avec succès.")
            else
                -- L'utilisateur n'existe pas
                TriggerClientEvent("esx:showNotification", source, "~r~Administration\n~s~Aucun joueur trouvé avec cet ID.")
            end
        end)
    else
...
function banplayerhistory(source, args)
