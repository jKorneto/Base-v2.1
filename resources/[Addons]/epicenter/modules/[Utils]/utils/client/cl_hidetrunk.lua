    ...
    while true do
        ...
        if inTrunk then
            ...
            if DoesEntityExist(vehicle) or not IsPedDeadOrDying(PlayerPedId()) or not IsPedFatallyInjured(PlayerPedId()) then
                ...
                DrawText3D(coords, '[~b~E~s~] pour quitter le coffre')
        ...
        then
            ...
            if trunk ~= -1 then
                ...
                if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), coords, true) <= 1.5 then
                    if not inTrunk then
                        if GetVehicleDoorAngleRatio(vehicle, 5) < 0.9 then
                            DrawText3D(coords, '[~b~E~s~] Se cacher\n[~b~H~s~] Ouvrir')
                    ...
                    if IsControlJustReleased(0, 51) and not inTrunk then
                        ...
                        if not exports["epicenter"]:IsInOtage() and not exports["epicenter"]:IsInPorter() then
                            if lockStatus == 1 then
...
function DrawText3D(coords, text)
    ...
    local dist = #(vector3(coords.x, coords.y, coords.z) - camCoords)
