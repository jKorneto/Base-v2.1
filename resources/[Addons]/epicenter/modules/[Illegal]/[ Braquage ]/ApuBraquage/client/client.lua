                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                wait = 1
                                DrawText3DApu(GetEntityCoords(bag).x, GetEntityCoords(bag).y, GetEntityCoords(bag).z, "Appuyez sur [~b~X~s~] pour ramasser le sac", 12)
...
AddEventHandler("initializePoliceBlip2", function(ApuIndex)
    CreateThread(function()
        ...
        local ApuVector = vector3(ApuIndex.x, ApuIndex.y, ApuIndex.z) 
        ...
        AddTextComponentString('[~r~Alerte~s~] Braquage')
        EndTextCommandSetBlipName(blip)
        Citizen.Wait(60000) -- 60 secondes
        RemoveBlip(blip)
    end)
end)
                        ...
                        if not eSup.IsRobbing then
                            ...
                            if canRob == true then
                                ...
                                if chance >= 0 and chance <= 25 then
                                    ...
                                    eSup.BarUpdate = AddTimerBar("Intimidation",{percentage = 0.0, bg = {155, 155, 155}, fg = {155,155,155,255}})
