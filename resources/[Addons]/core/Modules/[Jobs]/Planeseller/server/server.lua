...
RegisterNetEvent("Core:Planeseller:GetCat", function()
    ...
    if xPlayer then
        if xPlayer.job.name ~= "planeseller" then
            return xPlayer.ban(0, "(Core:Planeseller:GetCat) (job)")
        end

        if #(coords - Config["Planeseller"]["Office"]) < 15 then
            local SocietyCategories = {}
            for name, category in pairs(Categories) do
                if category.society == "planeshop" then
                    SocietyCategories[name] = category
                end
            end

            TriggerClientEvent("Core:Planeseller:ReceiveCat", xPlayer.source, SocietyCategories)
        else
            return
        end
    end
...
RegisterNetEvent("Core:Planeseller:GetVehForCat", function(CategoryName)
    local xPlayer = ESX.GetPlayerFromId(source)
    local player = GetPlayerPed(source)
    local coords = GetEntityCoords(player)
    ...
    if xPlayer then
        ...
        if #(coords - Config["Planeseller"]["Office"]) < 15 then
            ...
            TriggerClientEvent("Core:Planeseller:ReceiveVehForCat", xPlayer.source, VehCategories)
        else
            return
        end
    end
...
RegisterNetEvent("Core:Planeseller:SendBill", function(Target, Model)
    local xPlayer = ESX.GetPlayerFromId(source)
    local xTarget = ESX.GetPlayerFromId(Target)

    if (xPlayer.job.name ~= "planeseller") then
        return DropPlayer(source, "Veuillez vous reconnecter. Erreur : (Core:Planeseller:SendBill) #1")
    end
    ...
    if #(PCoords - Config["Planeseller"]["Office"]) <= 15 then
        WithinDistance = true
    end
    ...
    if VehiclePrice then
        local Bill = ESX.CreateBill(xPlayer.source, xTarget.source, BillPrice, "Achat de Véhicule en Concession", "society", "planeseller")
            ...
            generateUniquePlate(function(UniquePlate)
                MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle, type, state, boutique, stored) VALUES (@owner, @plate, @vehicle, @type, @state, @boutique, @stored)', {
                    ...
                    if rowsChanged > 0 then
                        ESX.RemoveSocietyMoney("planeseller", VehiclePrice)
                        xTarget.showNotification(("Vous avez recu un véhicule avec la plaque suivante [~b~%s~s~]"):format(UniquePlate))
                        ESX.SpawnVehicle(Model, Config["Planeseller"]["SpawnPos"], Config["Planeseller"]["SpawnHeading"], UniquePlate, true, xTarget, xTarget.identifier, function(vehicle) end)
                        CoreSendLogs(
                            "Planeseller",
                            "OneLife | Planeseller",
                            ...
                            Config["Log"]["Job"]["planeseller"]["sell_vehicle"]
                        )
                    else
...
RegisterNetEvent("iZeyy:Planeseller:SendAnnoucement", function(announcementType)
    local xPlayer = ESX.GetPlayerFromId(source)

    if (xPlayer) then
        if (xPlayer.job.name ~= "planeseller") then
            xPlayer.ban(0, "Veuillez vous reconnectez Erreur : (iZeyy:Planeseller:SendAnnoucement)")
            return
        end
        ...
        local announcementContent = Config["Planeseller"]["Annoucement"][announcementCategory]

        showSocietyNotify(xPlayer, xPlayer.job.name, "Concessionaire Avions", announcementContent, "Informations", 10)
    end  
end)