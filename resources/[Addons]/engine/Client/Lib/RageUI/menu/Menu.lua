...
UIMenu = Class.new(function(class)
    ...
    function self:Constructor(Title, Subtitle, X, Y, TextureDictionary, TextureName, R, G, B, A)
        ...
        self.Display.Header = (Engine["Config"]["Menu"] and Engine["Config"]["Menu"]["ShowHeader"]) or Engine["Config"]["Menu"]["ShowHeader"] == nil;
        self.Display.Glare = (Engine["Config"]["Menu"] and Engine["Config"]["Menu"]["DisplayGlare"]) or Engine["Config"]["Menu"]["DisplayGlare"] == nil;
        self.Display.Subtitle = (Engine["Config"]["Menu"] and Engine["Config"]["Menu"]["DisplaySubtitle"]) or Engine["Config"]["Menu"]["DisplaySubtitle"] == nil;
        self.Display.Background = (Engine["Config"]["Menu"] and Engine["Config"]["Menu"]["DisplayBackground"]) or Engine["Config"]["Menu"]["DisplayBackground"] == nil;
        self.Display.Navigation = (Engine["Config"]["Menu"] and Engine["Config"]["Menu"]["DisplayNavigationBar"]) or Engine["Config"]["Menu"]["DisplayNavigationBar"] == nil;
        self.Display.InstructionalButton = (Engine["Config"]["Menu"] and Engine["Config"]["Menu"]["DisplayInstructionalButton"]) or Engine["Config"]["Menu"]["DisplayInstructionalButton"] == nil;
        self.Display.PageCounter = (Engine["Config"]["Menu"] and Engine["Config"]["Menu"]["DisplayPageCounter"]) or Engine["Config"]["Menu"]["DisplayPageCounter"] == nil;

        self.Title = (Engine["Config"]["Menu"] and Engine["Config"]["Menu"]["Titles"]) or Title or ""
        self.TitleFont = ServerFontStyle
        self.TitleScale = 1.2
        ...
        self.Index = 1
        self.Sprite = {
            Dictionary = (Engine["Config"]["Menu"] and Engine["Config"]["Menu"]["TextureDictionary"]) or TextureDictionary or "commonmenu",
            Texture = (Engine["Config"]["Menu"] and Engine["Config"]["Menu"]["TextureName"]) or TextureName or "interaction_bgd",
            Color = {
                R = (Engine["Config"]["Menu"] and Engine["Config"]["Menu"]["Color"]["R"]) or R or 255,
                G = (Engine["Config"]["Menu"] and Engine["Config"]["Menu"]["Color"]["G"]) or G or 255,
                B = (Engine["Config"]["Menu"] and Engine["Config"]["Menu"]["Color"]["B"]) or B or 255, A = (Engine["Config"]["Menu"] and Engine["Config"]["Menu"]["Color"]["A"]) or A or 255
            }
        }
        self.Rectangle = nil
        ...
        self.data = {}
        self.ButtonColor = {
            R = (Engine["Config"]["MarkerRGB"]["R"]) or 255,
            G = (Engine["Config"]["MarkerRGB"]["G"]) or 255,
            B = (Engine["Config"]["MarkerRGB"]["B"]) or 255,
            A = (Engine["Config"]["MarkerRGB"]["A1"]) or 255
        }
    ...
    ---@return nil
    ---@public
    function self:SetButtonColor(R, G, B, A)
            ...
            if RageUI.GetCurrentMenu() == nil and not IsPauseMenuActive() then
                ...
                CreateThread(function()
                    while RageUI.CurrentMenu ~= nil do
                        ...
                        self:GetSubMenus();
                        if (not indexHasSet) then
                            self:SetIndex(startIndex or 1)
                            RageUI.SetCurrentIndex(startIndex or 1)
                            indexHasSet = true
                        end
                        Wait(1)
                    end
    ...
    function self:Toggle(startIndex)
        CreateThread(function()
    ...
    function self:RemoveInstructionButton(button)
        if type(button) == "table" then
            for i = 1, #self.InstructionalButtons do
                if Shared.Table:TableContains(self.InstructionalButtons[i], button[1]) then
