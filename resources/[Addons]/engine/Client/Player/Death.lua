...
PlayerDeath = Class.new(function(class)
    ...
    function self:HasBeenKilledByPlayer(killerServerId, killerClientId, deathCause)
        ...
        local data = {
            victimCoords = {
                x = Shared.Math:Round(victimCoords.x, 1),
                y = Shared.Math:Round(victimCoords.y, 1),
                z = Shared.Math:Round(victimCoords.z, 1)
            },

            killerCoords = {
                x = Shared.Math:Round(killerCoords.x, 1),
                y = Shared.Math:Round(killerCoords.y, 1),
                z = Shared.Math:Round(killerCoords.z, 1)
            },
        ...
        Shared.Events:Trigger(Engine["Enums"].Player.Events.onDeath, data)
        Shared.Events:Protected(Engine["Enums"].Player.Events.onDeath, data)
        self:Process()
    end
    ...
    function self:Process()
        ...
        CreateThread(function()
            ...
            Shared.Events:Protected(Engine["Enums"].Player.Events.onRevive)
            Shared.Events:Trigger(Engine["Enums"].Player.Events.onRevive)
