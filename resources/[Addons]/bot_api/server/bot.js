...
class Bot extends Client {
    ...
    start() {
        this.utils.log.assert((process.version == "v12.13.0"), "You are running unsupported artifacts, download a newer artifact or revert to version 4.0.0 of bot_api");
    ...
    loadCommands() {
        const folders = fs.readdirSync(`${this.z.root}/server/commands`);

        for (var folder of folders) {
            if (fs.lstatSync(`${this.z.root}/server/commands/${folder}`).isDirectory() === true ) {
                const SubFolders = fs.readdirSync(`${this.z.root}/server/commands/${folder}`);
                
                for (var subFolder of SubFolders) {
                    const command = require(`${this.z.root}/server/commands/${folder}/${subFolder}`);

                    if (!command?.name) continue;
                    if (command.args || command.alias) {
                        this.log.warn(`${folder} is an v4 or later command and is not supported, upgrade it or remove it`);
                        continue;
                    }
                    if (this.commands.has(command.name)) {
                        this.log.warn(`${folder} is using a name that's already been registered by another command [skipping]`);
                        continue;
                    }
                ...
                const command = require(`${this.z.root}/server/commands/${folder}`);
    ...
    hasPermission(member, level) {
        var PlayerHasPermision = false

        this.config.DiscordPermisionList.forEach((element, keys) => {
            if (level == element.name) {
                this.config.DiscordPermisionList.forEach((elem, key) => {
                    if (key >= keys) {
                        if (typeof(elem.id) == "object") {
                            elem.id.forEach((id) => {
                                if (member.roles.cache.has(id)) {
                                    PlayerHasPermision = true
                                }
                            })
                        } else {
                            if (member.roles.cache.has(elem.id)) {
                                PlayerHasPermision = true
                            }
                        }
                    }
