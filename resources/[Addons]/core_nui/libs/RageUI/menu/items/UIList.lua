    ...
    if CurrentMenu ~= nil then
        if CurrentMenu then
            ...
            if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
                ...
                local ListText = (type(Items[Index]) == "table") and string.format("← %s%s~s~ →", "~s~", Items[Index].value) or string.format("← %s%s~s~ →", "~s~", Items[Index]) or "undefined"
                ...
                if Enabled == true or Enabled == nil then
                    if Selected then
                        ListText = (type(Items[Index]) == "table") and string.format("← %s%s~s~ →", "~s~", Items[Index].value) or string.format("← %s%s~s~ →", "~s~", Items[Index]) or "undefined"
                    ...
                    if haveRightBadge then
                        ...
                        RenderSprite(RightBadge.BadgeDictionary or "commonmenu", RightBadge.BadgeTexture or "", CurrentMenu.X + SettingsButton.RightBadge.X + 55 + CurrentMenu.WidthOffset, CurrentMenu.Y + SettingsButton.RightBadge.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, SettingsButton.RightBadge.Width, SettingsButton.RightBadge.Height, 0, RightBadge.BadgeColour and RightBadge.BadgeColour.R or 255, RightBadge.BadgeColour and RightBadge.BadgeColour.G or 255, RightBadge.BadgeColour and RightBadge.BadgeColour.B or 255, RightBadge.BadgeColour and RightBadge.BadgeColour.A or 255)
                    end

                    RenderText(Label, CurrentMenu.X + SettingsButton.Text.X - 6 + LeftBadgeOffset, CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 0, SettingsButton.Text.Scale, 163, 159, 148, 255)
