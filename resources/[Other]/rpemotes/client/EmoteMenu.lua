...
function AddEmoteMenu(menu)
    ...
    if Config.SharedEmotesEnabled then
        sharemenu.OnItemSelect = function(sender, item, index)
            if ShareTable[index] ~= 'none' then
                ...
                if (distance ~= -1 and distance < 3) then
                    TriggerServerEvent("server:emote:request", GetPlayerServerId(target), ShareTable[index])
        ...
        shareddancemenu.OnItemSelect = function(sender, item, index)
            local target, distance = GetClosestPlayer()
            if (distance ~= -1 and distance < 3) then
                TriggerServerEvent("server:emote:request", GetPlayerServerId(target), DanceTable[index], 'Dances')
                SimpleNotify(Translate('sentrequestto') .. GetPlayerName(target))
            else
                SimpleNotify(Translate('nobodyclose'))
            end
...
if Config.Search then
    ...
    function EmoteMenuSearch(lastMenu)
        ClosePedMenu()
        local favEnabled = Config.FavKeybindEnabled
            ...
            if #results > 0 then
                ...
                searchMenu.OnItemSelect = function(sender, item, index)
                    ...
                    elseif data.table == "Shared" then
                        local target, distance = GetClosestPlayer()
                        if (distance ~= -1 and distance < 3) then
                            TriggerServerEvent("server:emote:request", GetPlayerServerId(target), data.name)
                            SimpleNotify(Translate('sentrequestto') .. GetPlayerName(target))
                        else
                            SimpleNotify(Translate('nobodyclose'))
                        end
                ...
                if Config.SharedEmotesEnabled then
                    if #sharedDanceMenu.Items > 0 then
                        ...
                        sharedDanceMenu.OnItemSelect = function(sender, item, index)
                            ...
                            local target, distance = GetClosestPlayer()
                            if (distance ~= -1 and distance < 3) then
                                TriggerServerEvent("server:emote:request", GetPlayerServerId(target), data.name, 'Dances')
                                SimpleNotify(Translate('sentrequestto') .. GetPlayerName(target))
                            else
                                SimpleNotify(Translate('nobodyclose'))
                            end
