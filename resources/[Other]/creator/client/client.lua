...
local function openCharacterCreator(sex, isAlreadyHaveSkin)
    ...
    if isAlreadyHaveSkin then
        ...
        elseif Config.SkinManager == "qb-clothing" then
            QBCore.Functions.TriggerCallback('creator:getCurrentSkin', function(skin)
...
function DeleteSkinCam(isCanceled)
    ...
    if playerHasAlreadySkin then
        ...
        ESX.ShowNotification("Bienvenue sur OneLife louer un Véhicule ou n'hésitez a prendre le Métro pour vous déplacer")
...
RegisterNetEvent('creator:openCreator')
AddEventHandler('creator:openCreator', function(sex, isAlreadyHaveSkin)
    lastCoords = {
...
StartLoop = function()
	...
	CreateThread(function()
        ...
        FreezeEntityPosition(PlayerPedId(), false)
        TriggerEvent('qb-spawn:client:openUI', src)
